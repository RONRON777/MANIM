...
PY
  then
    fail "화면 프로그램(PySide6) 설치가 되지 않았습니다." \
         "인터넷 연결 후 ./start.sh 를 다시 실행해주세요."
  fi
}
...
run_app() {
  if ! ./run.sh app >"$TMP_DIR/start_app.log" 2>&1; then
    if grep -q "MANIM_DB_KEY / MANIM_ENCRYPTION_KEY are required" "$TMP_DIR/start_app.log"; then
      fail "암호화 키를 읽지 못했습니다." \
           "파일 .env.local 을 확인하고 다시 실행해주세요."
    fi

    if grep -q "PySide6가 설치되지 않았습니다" "$TMP_DIR/start_app.log"; then
      fail "화면 프로그램(PySide6)이 없어 실행할 수 없습니다." \
           "인터넷 연결 후 ./start.sh 를 다시 실행해주세요."
    fi

    if grep -q "SQLCipher is required but unavailable" "$TMP_DIR/start_app.log"; then
      fail "DB 암호화 모듈 설정 문제입니다." \
           "config/security.yaml 의 allow_sqlite_fallback 값을 true 로 유지해주세요."
    fi

    if grep -q "Abort trap: 6" "$TMP_DIR/start_app.log" || \
       grep -q \
         "Connection Invalid error for service com.apple.hiservices-xpcservice" \
         "$TMP_DIR/start_app.log"; then
      fail "화면(GUI) 세션 연결 문제로 실행되지 않았습니다." \
           "Mac 로컬 데스크톱 터미널에서 실행해주세요."
    fi
    ...
    if grep -q "Incompatible processor" "$TMP_DIR/start_app.log" || \
      ...
      fail "PySide6 아키텍처가 현재 환경과 맞지 않습니다." \
           "가상환경 재생성 후 start.sh 재실행이 필요합니다."
    fi
